...
// add private data and methods here
  ...
  int numb_NonEmptyBuckets() const;

  //return the longest chain's length in this table
  int length_LongestChain() const;

  //contain the number of lists' entries in each bucket
  int *numb_EntriesInLists;


  //list pointers in each bucket
  ListType *listData;

  //initialize array of int << the number of entries for buckets
  void initialListData(ListType *listData);


  //initialize array of listData
  void initialNumb_EntriesInArray(int *array);
